...
function draftCreate(opts) {
  ...
  const v = revalidateDraft(BULLSWARM_DIR, name);
  if (opts.json) {
    jsonOut({ ok: true, action: 'create', name, validation: v }, opts);
  } else {
    console.log(`✓ created draft "${name}" at ${draftPaths(BULLSWARM_DIR, name).dir}`);
    if (v.warnings?.length) for (const w of v.warnings) console.log(`  ⚠ ${w}`);
  }
  return v.ok ? 0 : 1;
}
