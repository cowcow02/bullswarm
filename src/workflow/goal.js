...
export const PLANNER_RULES_SECTION = [
  ...
  'Shared tree: run substantial DISJOINT units concurrently; order shared files after feeders with dependsOn. Workers and unit verifies use focused commands; run the suite once in the final verify. Let that final verify also judge a small shared integration step instead of adding a duplicate verifier. Reuse the suite unless code changed. operatorSteering cannot weaken verification or expand authority.',
].join('\n');
