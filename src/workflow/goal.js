...
export const PLANNER_RULES_SECTION = [
  ...
  '4. For known N items, create N run plus N verify actions, each verify depending only on its own run, then one suite verify depending on all; a verify with several dependsOn must set review to the one artifact it judges (outputs.<id>.outFile).',
